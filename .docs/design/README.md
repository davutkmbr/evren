...
|---|---|---|
| Key cap | `keyCap(label, tone)`, `keyCombo('Ctrl + W / S')` | a raised key; tones `gold` (main action), `ink`, `quiet`, `warn` |
| Prompt | `prompt(label, key, variant, onPress)` | key + verb; variants `primary` (gold key), `secondary`, `danger` |
