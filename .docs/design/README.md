...
- **HUD zones (`src/ui/zones`):** the HUD composes itself. Every transient message (area title, race intro /
  ...
  | `lowerCenter` | one line, bottom edge gutter + 158 px (grows upwards for the hover panel) | the shared hint line (key hints, optional caption; the "[L] Kon" prompt and the viewing keys; the ferry escort's "[Z] Vapura eşlik et" and "Vapurdan uzaklaşıyorsun · [Z] Eşliği bırak"; the contextual move hints), maneuver and shot captions, hover controls, a moment's subtitle line (italic, shadowed, no box, slow fades) |
