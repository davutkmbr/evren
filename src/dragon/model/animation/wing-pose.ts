...
export function computeWingAngles(pose: Readonly<DragonPose>, side: number, out: WingAngles): void {
  ...
  const elevAmp = 0.62 * (1 - 0.25 * flare);
  const sweepAmp = 0.24 + 0.3 * flare;

  // Spread (glide) base + flap cycle.
  set(
    out.humerus,
    0.05 - 0.3 * amp * sinP + 0.42 * flare - 0.07 * twist,
    -0.02 - sweepAmp * amp * Math.cos(psi - 0.6) - 0.5 * dive + 0.32 * flare - 0.12 * twist,
    0.12 + amp * (elevAmp * cosP + 0.0) - 0.12 * dive + 0.28 * flare - 0.1 * twist,
  );
  set(
    out.forearm,
    -0.02 * amp,
  ...
  set(
    out.hand,
    -0.03 - 0.12 * amp * sinP - 0.2 * twist + 0.15 * flare,
  ...
  for (let i = 0; i < FINGERS.length; i++) {
    ...
    set(out.fingerB[i], 0, 0.04 * amp * fold(psi - 0.7), 0.16 * amp * Math.sin(psi - 0.7) - 0.02);
  }
}
