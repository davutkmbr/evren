...
/** Ground tiles per side: triangles are grouped so the renderer can frustum-cull each tile on its own. */
const TILES = 6;

/**
 * Reorders the mesh index by tile (TILE x TILE over the vertex extent) and returns the tile table: per tile the
 * first index, index count and bounding sphere (cx, cy, cz, r), 6 floats each; empty tiles are left out.
 */
export function tileIndex(positions: Float32Array, index: Uint32Array): { index: Uint32Array; tiles: Float32Array } {
  let x0 = Infinity;
  let z0 = Infinity;
  let x1 = -Infinity;
  let z1 = -Infinity;
