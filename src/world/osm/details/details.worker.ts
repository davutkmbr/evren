...
serveWorker<DetailsRequest, DetailsResult>((req) => {
  ...
  const walk = buildWalkGraph(data, { surface, cover, area, pads, poi, deck: req.deck, trunks: trees.trunks });
  const t3 = performance.now();
  const boats = buildBoats(data, surface.geo);
  const stamper = new PropStamper();
  const placer = new Placer({ surface, cover, area, pads, poi }, stamper);
